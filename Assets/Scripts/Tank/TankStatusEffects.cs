...
using TMPro;
using UnityEngine;
public class TankStatusEffects : NetworkBehaviour
{

    public static string GenerateEffectId(string ownerId, string effectName)
    {
        return $"{ownerId}_{effectName[(effectName.LastIndexOf('/') + 1)..]}";
    }
    ...
    private readonly Dictionary<string, StatusEffect> _localStatusEffectInstances = new();
    ...
    private void UpdateStatusEffectInstances()
    {
        ...
        foreach (var effectToApply in _statusEffects.Where(effect => !_localStatusEffectInstances.ContainsKey(effect.Key)))
            if (isServer)
                ServerApplyStatusEffect(effectToApply.Key, effectToApply.Value.statusEffectName, effectToApply.Value.fromConnId, effectToApply.Value.fromTeamId);
            else
    ...
    private void ClientApplyStatusEffect(string id, StatusEffectIdentity statusEffect)
    {
        ...
        if (effectAsset == default || effectAsset.type != GameStatusEffectsManager.EffectType.Tank)
            return;
        var effectObject = Instantiate(effectAsset.prefab, effectObjectsParent);
        if (!effectObject.TryGetComponent(out StatusEffect statusEffectInstance))
        {
            Destroy(effectObject);
            return;
        }
        statusEffectInstance.EffectTarget = new StatusEffect.StatusEffectTarget
        {
            Tank = tank,
        ...
        statusEffectInstance.SetFromTeamId(statusEffect.fromTeamId);
    ...
    [Server]
    public void ServerApplyStatusEffect(string id, string statusEffectName, int? fromConnId, string fromTeamId = null)
    {
        var statusEffectIdentity = new StatusEffectIdentity
        {
        ...
        else
        {
            ...
            _localStatusEffectInstances[id] = statusEffectInstance;
            statusEffectInstance.SetFromTeamId(fromTeamId);
            statusEffectIdentity.instanceDataJson = statusEffectInstance.ServerInit(fromConnId);
        }
    ...
    {
        return _statusEffects.TryGetValue(id, out var result) ? result.statusEffectName : "";
    }
